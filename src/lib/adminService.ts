import { supabase, BlogPost, Booking, Contact } from './supabase';
...
export const contactService = {
  ...
  async updateStatus(id: string, status: 'new' | 'reviewed' | 'replied' | 'closed'): Promise<boolean> {
    try {
      const { error } = await supabase
        .from('contacts')
        .update({ status, updated_at: new Date().toISOString() })
    ...
    try {
      return supabase
        ...
        .on(
          ...
          (payload) => {
            console.log('Contacts change:', payload);
            callback(payload);
          }
        )
        .subscribe();
  ...
  // Unsubscribe from contacts changes
  unsubscribeFromChanges(channel: RealtimeChannel | null): void {
    if (channel) {
      supabase.removeChannel(channel);
    }
